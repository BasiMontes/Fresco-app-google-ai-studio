        ...
        {view === 'auth' ? <AuthPage onLogin={() => {}} onSignup={() => {}} /> : 
         ...
         <>
          ...
          <main className="flex-1 h-screen overflow-y-auto bg-[#F4F4F4] relative">
            ...
            {/* NAVBAR MÓVIL OPTIMIZADA ( py-6.5 en lugar de py-9 ) */}
                ...
                {navItems.map((item, idx) => {
                    ...
                    return (
                      <button 
                        key={item.id} 
                        onClick={() => setActiveTab(item.id)} 
                        className="flex-1 flex flex-col items-center justify-center py-6.5 relative group transition-all duration-300 z-10 active:scale-75"
                      >
                          <item.icon 
                            strokeWidth={3}
                            className={`w-[26px] h-[26px] transition-all duration-500 ${isActive ? 'text-white scale-110 active-icon-glow' : 'inactive-icon-style hover:scale-110'}`} 
                          />
                      </button>
                    );
                })}
