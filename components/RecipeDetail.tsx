...
import { X, Clock, ChefHat, PlayCircle, ShoppingCart, CheckCircle, Minus, Plus, RefreshCw, Trash2, Sunrise, Sun, Moon, AlertTriangle, ListChecks, UtensilsCrossed, Heart } from 'lucide-react';
  ...
  return (
    <div className="fixed inset-0 z-[2000] bg-black/60 backdrop-blur-sm animate-fade-in flex justify-center items-center p-3 md:p-6" onClick={onClose}>
        <div className="bg-white w-full max-w-5xl max-h-[92vh] rounded-[2rem] overflow-hidden flex flex-col relative shadow-2xl animate-slide-up" onClick={e => e.stopPropagation()}>
            ...
            {/* Bottom Floating Bar - More Compact */}
                ...
                {showPlanningMode ? (
                    <div className="flex flex-col md:flex-row items-stretch md:items-center gap-3 w-full animate-slide-up">
                        <div className="flex-1 flex gap-2">
                            <input type="date" value={planDate} onChange={e => setPlanDate(e.target.value)} className="flex-1 bg-gray-50 border border-gray-200 rounded-xl px-4 py-3 text-sm font-bold text-teal-900 outline-none focus:ring-2 focus:ring-teal-500" />
                            <div className="flex bg-gray-100 p-1 rounded-xl">
                                {(['breakfast', 'lunch', 'dinner'] as MealCategory[]).map(cat => (
                                    <button key={cat} onClick={() => setPlanType(cat)} className={`px-4 rounded-lg transition-all ${planType === cat ? 'bg-white text-teal-900 shadow-sm' : 'text-gray-400'}`}>
                        ...
                        <div className="flex gap-2">
                            <button onClick={() => setShowPlanningMode(false)} className="px-6 py-3 font-black text-[10px] uppercase tracking-widest text-gray-400 hover:text-gray-600 transition-colors">Cancelar</button>
                            <button onClick={() => { onAddToPlan?.(desiredServings, planDate, planType); onClose(); }} className="px-10 py-3.5 bg-teal-900 text-white rounded-xl font-black text-[10px] uppercase tracking-widest shadow-lg hover:bg-teal-800 active:scale-95 transition-all">Confirmar Plan</button>
                        </div>
                    </div>
                ) : (
