...
export const Planner: React.FC<PlannerProps> = ({ user, plan, recipes, pantry, onUpdateSlot, onAIPlanGenerated, onClear }) => {
  ...
  return (
    <div className="h-full w-full flex flex-col animate-fade-in overflow-hidden bg-[#F4F4F4] md:bg-[#FDFDFD]">
      <header className="w-full py-4 md:py-6 bg-white border-b border-gray-100 flex-shrink-0 z-20 px-4 md:px-8">
        <div className="w-full grid grid-cols-1 md:grid-cols-3 items-center gap-2 md:gap-0">
            
            <div className="justify-self-start flex flex-col">
                <h1 className="text-[22px] md:text-[28px] font-black text-[#0F4E0E] tracking-tight leading-none">Calendario</h1>
                <p className="text-[#0F4E0E]/20 font-black uppercase text-[8px] tracking-[0.4em] mt-1 hidden md:block">Planificación Semanal</p>
            </div>

            <div className="flex items-center gap-1 bg-gray-50/80 p-1 rounded-2xl md:justify-self-center md:min-w-[280px] w-full md:w-auto my-1 md:my-0">
                <button 
                    disabled={!canGoBack}
                ...
                <span className="flex-1 text-[10px] font-black text-[#0F4E0E] px-4 uppercase tracking-[0.25em] whitespace-nowrap text-center">
            ...
            <div className="justify-self-end flex items-center gap-2 h-10 w-full md:w-auto mt-2 md:mt-0">
                <button 
                ...
                <button 
                    ...
                    className="flex-1 md:flex-none h-10 px-5 bg-[#0F4E0E] text-white rounded-xl font-black text-[9px] uppercase tracking-[0.2em] shadow-lg hover:bg-[#062606] transition-all active:scale-[0.98] flex items-center justify-center gap-2 border-b-2 border-black/20"
                >
      ...
      <div ref={scrollContainerRef} className="flex-1 overflow-x-auto no-scrollbar flex gap-0 md:gap-4 p-0 md:p-6 h-full min-h-0 bg-[#F4F4F4] md:bg-[#FCFCFC]">
        {days.map((day) => {
          const dateStr = format(day, 'yyyy-MM-dd');
          const isToday = isSameDay(day, new Date());
          return (
            <div key={dateStr} className="min-w-screen md:min-w-[300px] w-full md:max-w-[340px] flex-shrink-0 flex flex-col gap-0 md:gap-4 h-full border-r border-gray-200/50 md:border-0 last:border-0">
                {/* DÍA HEADER - INTEGRADO EN MÓVIL */}
                <div className={`flex items-center justify-center gap-3 py-4 md:py-3 px-4 md:rounded-[2rem] transition-all duration-700 flex-shrink-0 ${isToday ? 'bg-[#0F4E0E] text-white shadow-xl z-10' : 'bg-white md:bg-white text-[#0F4E0E] border-b border-gray-100 md:border-gray-100 shadow-sm'}`}>
                    <span className={`text-[11px] font-black uppercase tracking-[0.3em] whitespace-nowrap ${isToday ? 'text-orange-400' : 'text-[#0F4E0E]/30'}`}>{format(day, 'EEEE', { locale: es })}</span>
                    <span className="text-xl font-black leading-none">{format(day, 'd')}</span>
                </div>
                
                {/* CONTENEDOR DE SLOTS - LLENANDO ESPACIO */}
                <div className="flex-1 flex flex-col gap-0.5 md:gap-4 p-0.5 md:p-0">
                  {(['breakfast', 'lunch', 'dinner'] as MealCategory[]).map((type) => {
                      ...
                      return (
                          <div 
                            ...
                            className={`relative flex-1 min-h-[140px] md:min-h-[125px] md:rounded-[2rem] transition-all duration-500 cursor-pointer overflow-hidden group shadow-none md:shadow-sm border-0 md:border-2 ${recipe ? 'bg-gray-100' : 'bg-white md:bg-white md:border-dashed border-gray-100 hover:bg-teal-50'}`}
                          >
                              {recipe ? (
                                  <div className="absolute inset-0 w-full h-full flex flex-col animate-fade-in">
                                      ...
                                      <div className="relative z-10 flex flex-col h-full justify-between p-5 md:p-4">
                                          <div className="flex justify-between items-start">
                                              <div className="px-2 py-1 rounded-lg bg-black/40 backdrop-blur-md border border-white/10 flex items-center">
                                                  <span className="text-[8px] font-black uppercase tracking-[0.2em] text-white leading-none">{type}</span>
                                              </div>
                                              <button 
                                                onClick={(e) => { e.stopPropagation(); onUpdateSlot(dateStr, type, undefined); }} 
                                                className="p-2 bg-white/10 backdrop-blur-md rounded-lg text-white/60 hover:text-white hover:bg-red-500 transition-all opacity-100 md:opacity-0 group-hover:opacity-100"
                                              >
                                          ...
                                          <div className="space-y-1 md:space-y-0.5">
                                              <h5 className="font-black text-[15px] md:text-[13px] text-white leading-tight uppercase line-clamp-2 drop-shadow-md">{recipe.title}</h5>
                                              <div className="flex items-center gap-2 opacity-100 md:opacity-0 group-hover:opacity-100 transition-all duration-500 transform translate-y-0 md:translate-y-1 md:group-hover:translate-y-0">
                                  ...
                                  <div className="w-full h-full flex flex-col items-center justify-center p-2 text-center animate-fade-in">
                                      <div className="flex flex-col items-center justify-center transition-all duration-500 group-hover:scale-105">
                                          <div className="w-10 h-10 md:w-8 md:h-8 rounded-xl bg-teal-50/50 flex items-center justify-center mb-2 border border-teal-100/30 group-hover:bg-[#0F4E0E] group-hover:text-white transition-all">
                                            <Plus className="w-5 h-5 md:w-4 md:h-4 text-[#0F4E0E] group-hover:text-white" />
                                          </div>
                                          <span className="text-[9px] md:text-[8px] font-black uppercase tracking-[0.4em] text-gray-300 group-hover:text-[#0F4E0E] transition-colors">{type}</span>
                                      </div>
                                  </div>
                              )}
                          </div>
      ...
      {showRecipeSelector && (
          <ModalPortal>
              <div className="fixed inset-0 z-[5000] bg-[#0F4E0E]/90 backdrop-blur-xl flex items-center justify-center p-4" onClick={() => setShowRecipeSelector(null)}>
                  <div className="bg-white w-full max-w-xl rounded-[3.5rem] p-8 md:p-10 shadow-2xl relative animate-slide-up flex flex-col max-h-[85vh] border border-white/20" onClick={e => e.stopPropagation()}>
                    <div className="flex justify-between items-start mb-6 md:mb-8">
                        <div>
                            <h3 className="text-2xl md:text-3xl font-black text-[#0F4E0E] tracking-tighter">Elegir Plato</h3>
                            <p className="text-[10px] text-[#0F4E0E]/40 font-black uppercase tracking-[0.3em] mt-2">
                    ...
                    <div className="relative mb-6 md:mb-8">
                        ...
                        <input 
                            ...
                            className="w-full h-14 md:h-16 pl-14 pr-6 bg-gray-50 border-2 border-transparent focus:border-teal-500/10 rounded-2xl font-bold text-base outline-none transition-all shadow-inner text-[#0F4E0E]"
                        />
                    </div>
                    ...
                    <div className="flex-1 overflow-y-auto no-scrollbar space-y-3 pr-2">
                        {filteredSelectorRecipes.map(recipe => (
                            <div 
                                ...
                                className="group flex items-center gap-4 md:gap-5 p-3 md:p-4 bg-white border border-gray-100 rounded-[2.2rem] hover:bg-teal-50 hover:border-teal-200 transition-all cursor-pointer shadow-sm hover:shadow-md"
                            >
                                <div className="w-14 h-14 md:w-16 md:h-16 rounded-2xl overflow-hidden shadow-md flex-shrink-0">
                                ...
                                <div className="flex-1 min-w-0">
                                    <h4 className="font-black text-[14px] md:text-[15px] text-[#0F4E0E] leading-tight mb-1 truncate">{recipe.title}</h4>
                                    <span className="text-[9px] font-black uppercase text-[#0F4E0E]/60 tracking-widest">{recipe.cuisine_type}</span>
                                </div>
                                <div className="w-10 h-10 bg-[#0F4E0E] rounded-2xl flex items-center justify-center opacity-0 group-hover:opacity-100 translate-x-4 group-hover:translate-x-0 transition-all text-white shadow-xl">
            ...
            <div className="bg-white w-full max-w-md rounded-[3.5rem] p-10 shadow-2xl relative animate-slide-up border border-white/20" onClick={e => e.stopPropagation()}>
              ...
              <div className="space-y-8">
                ...
                <button onClick={executeSmartPlan} disabled={isGenerating} className="w-full h-16 bg-[#0F4E0E] text-white rounded-[1.8rem] font-black uppercase tracking-[0.25em] shadow-2xl flex items-center justify-center gap-4 hover:bg-[#062606] transition-all active:scale-[0.97] hover:-translate-y-1">
                  {isGenerating ? <Loader2 className="w-6 h-6 animate-spin" /> : <><Wand2 className="w-5 h-5 text-orange-400" /> Generar</>}
                </button>
              </div>
            </div>
          </div>
