  ...
  useEffect(() => {
    const timer = setTimeout(() => {
        if (scrollContainerRef.current) {
            ...
            if (todayIndex !== -1) {
                const dayWidth = window.innerWidth < 768 ? window.innerWidth * 0.88 : 320; 
  ...
  return (
    <div className="h-full w-full flex flex-col animate-fade-in overflow-hidden bg-[#F8F9FA]">
      ...
      <div 
        ...
        className="flex-1 overflow-x-auto no-scrollbar flex gap-6 md:gap-8 p-5 md:p-10 bg-[#F8F9FA] scroll-smooth"
      >
        {days.map((day) => {
          ...
          return (
            <div 
              key={dateStr} 
              className="min-w-[85vw] md:min-w-[320px] lg:min-w-[340px] flex-shrink-0 flex flex-col gap-5 h-full animate-fade-in transition-all duration-500"
            >
                {/* CABECERA DEL DÍA */}
                <div className="flex items-center px-8 py-5 md:py-6 rounded-[2rem] md:rounded-[2.5rem] bg-white text-[#0F4E0E] shadow-sm border border-gray-100 flex-shrink-0">
                    ...
                    {isToday && (
                        <div className="ml-auto flex items-center gap-2">
                            ...
                            <div className="w-3 h-3 rounded-full bg-orange-500 shadow-[0_0_15px_rgba(232,124,62,0.8)] animate-pulse" />
                        </div>
                    )}
                </div>
                
                {/* LISTA DE COMIDAS - ELIMINADO EL GRID QUE APLASTABA LAS TARJETAS */}
                  ...
                  {(['breakfast', 'lunch', 'dinner'] as MealCategory[]).map((type) => {
                      ...
                      return (
                          <div 
                            ...
                            className={`relative w-full h-[200px] md:h-[240px] flex-shrink-0 rounded-[3rem] transition-all duration-700 cursor-pointer overflow-hidden group shadow-sm ${recipe ? 'bg-black' : 'bg-white border-2 border-dashed border-gray-100 hover:border-[#0F4E0E]/20'}`}
                          >
                              {recipe ? (
                                  <>
                                      {/* CAPA 0: IMAGEN A SANGRE TOTAL - CALCANDO OBJETIVO */}
                                      <SmartImage 
                                      ...
                                      {/* CAPA 1: DEGRADADO PARA LEGIBILIDAD EXTREMA */}
                                      <div className="absolute inset-0 bg-gradient-to-t from-black/95 via-black/30 to-transparent z-[1]" />
                                      
                                      {/* CAPA 2: CONTENIDO FLOTANTE (ESTO ES EL DISEÑO OBJETIVO) */}
                                      <div className="relative z-[2] flex flex-col h-full justify-between p-7 md:p-8">
                                          <div className="flex justify-between items-start">
                                              {/* PÍLDORA MOMENTO GLASS */}
                                              <div className="px-5 py-2.5 rounded-full bg-white/10 backdrop-blur-xl border border-white/10 flex items-center shadow-lg">
                                              ...
                                              <button 
                                                ...
                                                className="w-12 h-12 bg-white/20 hover:bg-red-500 text-white rounded-full backdrop-blur-md shadow-2xl transition-all active:scale-90 flex items-center justify-center border border-white/10 group-hover:opacity-100 md:opacity-0"
                                              >
                                          ...
                                          <div className="space-y-4">
                                              {/* TÍTULO BLANCO, EXTRA BOLD, CAPITALIZE - CALCANDO OBJETIVO */}
                                              <h5 className="font-black text-2xl md:text-3xl text-white leading-[1.05] capitalize line-clamp-2 tracking-tight drop-shadow-xl">
                                                {recipe.title}
                                              </h5>
                                              
                                              {/* PÍLDORAS INFERIORES OSCURAS GLASS - CALCANDO OBJETIVO */}
                                              <div className="flex items-center gap-3">
                                                  <div className="flex items-center gap-2 bg-black/50 px-4 py-3 rounded-2xl backdrop-blur-3xl border border-white/5 shadow-2xl">
                                                    <Clock className="w-4 h-4 text-white" />
                                                    <span className="text-[12px] font-black text-white uppercase tracking-widest">{recipe.prep_time} MIN</span>
                                                  </div>
                                                  <div className="flex items-center gap-2 bg-black/50 px-4 py-3 rounded-2xl backdrop-blur-3xl border border-white/5 shadow-2xl">
                                                    <BarChart2 className="w-4 h-4 text-white" />
                                                    <span className="text-[12px] font-black text-white uppercase tracking-widest">{recipe.difficulty || 'EASY'}</span>
                                                  </div>
                                              </div>
                                          </div>
                                      </div>
                                  ...
                                  <div className="w-full h-full flex flex-col items-center justify-center p-6 text-center animate-fade-in relative z-[2]">
                                      <div className="w-14 h-14 md:w-16 md:h-16 rounded-[1.8rem] bg-gray-50 flex items-center justify-center mb-4 border border-gray-50 group-hover:bg-[#0F4E0E] group-hover:text-white transition-all duration-700 group-hover:scale-110 group-hover:rotate-90">
