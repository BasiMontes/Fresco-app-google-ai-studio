...
export const Planner: React.FC<PlannerProps> = ({ user, plan, recipes, pantry, onUpdateSlot, onAIPlanGenerated, onClear }) => {
  ...
  return (
    <div className="h-full w-full flex flex-col animate-fade-in overflow-hidden bg-[#F8F9FA]">
      ...
      {/* VIEWPORT DE CALENDARIO: COLUMNAS QUE "RESPIRAN" */}
      <div 
      ...
      >
        {days.map((day) => {
          ...
          return (
            <div 
              key={dateStr} 
              className={`min-w-[82vw] md:min-w-[380px] flex-shrink-0 flex flex-col gap-6 md:gap-8 animate-fade-in transition-all duration-500 ${isToday ? 'scale-100' : 'opacity-80 scale-[0.97]'}`}
            >
                {/* CABECERA DEL DÍA */}
                ...
                {/* LISTA DE COMIDAS: TARJETAS CONSISTENTES */}
                  ...
                  {(['breakfast', 'lunch', 'dinner'] as MealCategory[]).map((type) => {
                      ...
                      return (
                          <div 
                            ...
                            className={`relative min-h-[160px] md:min-h-[220px] rounded-[3rem] transition-all duration-500 cursor-pointer overflow-hidden group border-2 ${recipe ? 'bg-gray-900 border-transparent shadow-xl' : 'bg-white border-dashed border-gray-100 hover:border-[#0F4E0E] hover:bg-teal-50/10'}`}
                          >
                              {recipe ? (
                                  <div className="absolute inset-0 w-full h-full flex flex-col animate-fade-in">
                                      ...
                                      <div className="relative z-10 flex flex-col h-full justify-between p-8 md:p-10">
                                          ...
                                          <div className="space-y-3">
                                              <h5 className="font-black text-xl md:text-2xl text-white leading-tight uppercase line-clamp-2 drop-shadow-2xl tracking-tighter">{recipe.title}</h5>
                                              <div className="flex items-center gap-3">
                                                  ...
                                                  <span className="text-[9px] font-black text-white/50 uppercase tracking-widest capitalize">{recipe.difficulty}</span>
                                              </div>
                                          </div>
                                      </div>
                                  </div>
                                  ...
                                  <div className="w-full h-full flex flex-col items-center justify-center p-8 text-center animate-fade-in">
                                      <div className="w-16 h-16 md:w-20 md:h-20 rounded-[2rem] bg-gray-50 flex items-center justify-center mb-4 border border-gray-50 group-hover:bg-[#0F4E0E] group-hover:text-white transition-all duration-500">
      ...
      {/* SELECTOR DE RECETA - DISEÑO MODAL PREMIUM */}
      {showRecipeSelector && (
          <ModalPortal>
              <div className="fixed inset-0 z-[5000] bg-[#0F4E0E]/95 backdrop-blur-3xl flex items-center justify-center p-4 md:p-8" onClick={() => setShowRecipeSelector(null)}>
                  <div className="bg-white w-full max-w-4xl rounded-[3.5rem] p-8 md:p-16 shadow-2xl relative animate-slide-up flex flex-col max-h-[90vh] border border-white/20" onClick={e => e.stopPropagation()}>
                    <div className="flex justify-between items-start mb-10">
                        <div>
                            <h3 className="text-4xl md:text-5xl font-black text-[#0F4E0E] tracking-tighter">¿Qué comemos?</h3>
                    ...
                    <div className="relative mb-8">
                        ...
                        <input 
                            ...
                            className="w-full h-16 md:h-20 pl-20 pr-10 bg-gray-50 border-2 border-transparent focus:border-[#0F4E0E]/10 rounded-[2.2rem] font-bold text-lg md:text-xl outline-none transition-all shadow-inner text-[#0F4E0E] placeholder:text-gray-200"
                        />
                    </div>

                    <div className="flex-1 overflow-y-auto no-scrollbar space-y-4 md:space-y-6 pr-2">
                        {filteredSelectorRecipes.map(recipe => (
                            <div 
                                ...
                                className="group flex items-center gap-6 md:gap-8 p-4 md:p-6 bg-white border border-gray-100 rounded-[2.5rem] md:rounded-[3rem] hover:bg-teal-50/20 hover:border-teal-100 transition-all cursor-pointer shadow-sm hover:shadow-xl"
                            >
                                <div className="w-20 h-20 md:w-28 md:h-28 rounded-[1.5rem] md:rounded-[2rem] overflow-hidden shadow-xl flex-shrink-0">
                                ...
                                <div className="flex-1 min-w-0">
                                    <h4 className="font-black text-xl md:text-2xl text-[#0F4E0E] leading-tight mb-2 md:mb-3 truncate tracking-tighter uppercase">{recipe.title}</h4>
                                ...
                                <div className="w-12 h-12 md:w-16 md:h-16 bg-[#0F4E0E] rounded-[1.4rem] md:rounded-[1.8rem] flex items-center justify-center opacity-0 group-hover:opacity-100 translate-x-4 group-hover:translate-x-0 transition-all text-white shadow-2xl flex-shrink-0">
                                    <Check className="w-6 h-6 md:w-8 md:h-8 stroke-[3px]" />
                                </div>
                            </div>
                        ))}
                    </div>
