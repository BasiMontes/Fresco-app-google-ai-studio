...
const MODAL_INPUT_CLASSES = "w-full h-[62px] px-6 bg-[#F9FAFB] rounded-[1.4rem] font-black text-[11px] text-[#0F4E0E] uppercase tracking-widest outline-none border-none transition-all focus:bg-gray-100 appearance-none flex items-center";
...
export const Pantry: React.FC<PantryProps> = ({ items, onRemove, onAdd, onUpdateQuantity, onAddMany, onEdit, isOnline = true }) => {
  ...
  const getExpiryStatus = (item: PantryItem) => {
    ...
    return { type: 'fresh', label: format(expiryDate, "d MMM", { locale: es }), color: 'text-[#0F4E0E]', icon: Clock };
  };
  ...
  return (
    <div className="animate-fade-in pb-48 w-full max-w-7xl mx-auto px-4 md:px-8 bg-[#FCFCFC] h-full overflow-y-auto no-scrollbar">
      <header className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 pt-6 mb-6">
        <div>
            <h1 className="text-[#0F4E0E] text-[2rem] font-black tracking-[-0.05em] leading-[0.9]">Mi Stock</h1>
        ...
        <div className="flex gap-2 w-full sm:w-auto">
            <div className="relative flex-1 sm:w-56">
                ...
                <input type="text" placeholder="Buscar..." value={searchTerm} onChange={(e) => { setSearchTerm(e.target.value); setVisibleLimit(ITEMS_PER_PAGE); }}
                    className="w-full pl-9 pr-4 py-2.5 bg-white border border-gray-100 rounded-xl text-[11px] font-bold text-[#0F4E0E] focus:outline-none shadow-sm transition-all"
                />
            </div>
            ...
            <button onClick={() => { setNewItem({ name: '', category: 'pantry', quantity: 1, unit: 'uds', added_at: format(new Date(), 'yyyy-MM-dd'), expires_at: '' }); setShowAddModal(true); }} className="p-2.5 bg-[#0F4E0E] text-white rounded-xl shadow-lg active:scale-95 transition-all"><Plus className="w-4 h-4" /></button>
        </div>
      </header>
      ...
      <div className="relative group/filters mb-6">
          {canScrollLeft && (
              <button 
                onClick={() => scroll('left')}
                className="absolute left-0 top-1/2 -translate-y-1/2 z-20 w-8 h-8 bg-white/90 backdrop-blur shadow-md rounded-full flex items-center justify-center text-[#0F4E0E] border border-gray-100 animate-fade-in"
              >
                  ...
                  return (
                      <button 
                        ...
                        className={`px-5 py-2.5 rounded-full font-bold text-[11px] whitespace-nowrap transition-all duration-300 border flex items-center gap-2 ${
                            isActive 
                            ? 'bg-[#e6f2f1] border-[#0F4E0E] text-[#0F4E0E] shadow-sm' 
                            : 'bg-[#f4f7f6] border-transparent text-[#6e8a88] hover:bg-gray-100'
                        }`}
                      >
                          <span>{opt.emoji}</span>
                          {opt.label}
                          <span className={`text-[9px] px-1.5 py-0.5 rounded-md ${isActive ? 'bg-[#0F4E0E] text-white' : 'bg-white/50 text-[#6e8a88]'}`}>{count}</span>
                      </button>
                  );
              })}
          </div>
          ...
          {canScrollRight && (
              <button 
                ...
                className="absolute right-0 top-1/2 -translate-y-1/2 z-20 w-8 h-8 bg-white/90 backdrop-blur shadow-md rounded-full flex items-center justify-center text-[#0F4E0E] border border-gray-100 animate-fade-in"
              >
      ...
      <div className="flex items-center gap-6 mb-8 px-2">
          <div className="flex items-center gap-1 group">
              ...
              <button onClick={() => setSortBy(prev => prev === 'name' ? 'expiry' : prev === 'expiry' ? 'quantity' : 'name')} className="flex items-center gap-2 text-[#4a5f6b] hover:text-[#0F4E0E] transition-colors">
                  <span className="text-[12px] font-bold uppercase tracking-wider">Sort by: <span className="text-[#0F4E0E] capitalize">{sortBy === 'expiry' ? 'Caducidad' : sortBy === 'quantity' ? 'Cantidad' : 'Nombre'}</span></span>
              </button>
          </div>
          <button onClick={() => { setFilterExpiring(!filterExpiring); setVisibleLimit(ITEMS_PER_PAGE); }} className={`flex items-center gap-3 transition-colors group ${filterExpiring ? 'text-orange-500' : 'text-[#4a5f6b] hover:text-[#0F4E0E]'}`}><CalendarClock className={`w-4 h-4 group-hover:scale-110 transition-transform ${filterExpiring ? 'text-orange-500' : 'text-[#4a5f6b]'}`} /><span className="text-[12px] font-bold uppercase tracking-wider">Expiring soon</span>{filterExpiring && <div className="w-1.5 h-1.5 rounded-full bg-orange-500 animate-pulse" />}</button>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
          {visibleItems.length === 0 ? (
              <div className="col-span-full py-20 text-center opacity-10 flex flex-col items-center">
                  ...
                  {selectedCategory !== 'all' && <button onClick={() => setSelectedCategory('all')} className="mt-4 text-[#0F4E0E] font-bold text-xs underline pointer-events-auto">Ver todo el stock</button>}
              </div>
          ) : (
                        ...
                        <div className="flex items-center bg-gray-50 rounded-xl p-0.5 border border-gray-100 flex-shrink-0 w-[110px] h-10">
                            ...
                            <div className="flex-1 text-center min-w-0 flex flex-col justify-center">
                                <input 
                                    type="text"
                                    inputMode="decimal"
                                    className="w-full bg-transparent font-black text-[13px] text-[#0F4E0E] leading-none text-center outline-none border-none p-0 focus:ring-0 tracking-tighter"
                                ...
                                />
                                <p className="text-[9px] font-black text-[#0F4E0E]/60 uppercase tracking-tighter leading-none mt-0.5">{(item.unit || 'uds').toUpperCase()}</p>
                            </div>
                            <button onClick={(e) => handleAdjust(e, item, 1)} className="w-8 h-8 flex items-center justify-center text-[#0F4E0E] hover:text-[#062606] transition-colors"><Plus className="w-3.5 h-3.5" /></button>
                        </div>
                        
                        <button onClick={() => setItemToEdit(item)} className="p-1.5 text-gray-300 hover:text-[#0F4E0E] transition-colors flex-shrink-0">
      ...
      {visibleLimit < filteredItems.length && (
          <div className="flex justify-center pt-10"><button onClick={() => setVisibleLimit(prev => prev + ITEMS_PER_PAGE)} className="group flex items-center gap-4 px-8 py-3 bg-white border border-gray-100 rounded-2xl shadow-md hover:shadow-lg transition-all"><span className="text-[#0F4E0E] font-black text-[10px] uppercase tracking-widest">Ver más inventario</span><ChevronDown className="w-4 h-4 text-gray-200 group-hover:translate-y-1 transition-transform" /></button></div>
      )}
        ...
        <ModalPortal>
          <div className="fixed inset-0 z-[5000] bg-black/40 backdrop-blur-xl flex items-center justify-center p-4" onClick={() => setItemToEdit(null)}>
              <div className="w-full max-w-[420px] bg-white rounded-[2.8rem] p-10 shadow-2xl relative animate-slide-up" onClick={e => e.stopPropagation()}>
                  <div className="flex justify-between items-center mb-8"><h2 className="text-[#0F4E0E] text-[2rem] font-black tracking-tight leading-none">Editar Item</h2><div className="flex gap-1"><button onClick={() => { triggerDialog({ title: '¿Borrar?', message: 'Se perderá el stock.', type: 'confirm', onConfirm: () => { onRemove(itemToEdit.id); setItemToEdit(null); } }); }} className="p-2.5 text-gray-200 hover:text-red-500 transition-colors"><Trash2 className="w-6 h-6" /></button><button onClick={() => setItemToEdit(null)} className="p-2.5 text-gray-200 hover:text-black transition-colors"><X className="w-7 h-7" /></button></div></div>
                  <div className="space-y-6">
                      ...
                      <div className="grid grid-cols-2 gap-4">
                          ...
                          <div className="flex flex-col">
                              ...
                              <div className="relative">
                                  <input type="text" className={MODAL_INPUT_CLASSES + " !text-[#0F4E0E]"} placeholder="AAAA-MM-DD" value={itemToEdit.expires_at ? format(new Date(itemToEdit.expires_at), "yyyy-MM-dd") : ""} onChange={e => setItemToEdit({...itemToEdit, expires_at: new Date(e.target.value).toISOString()})} />
                      ...
                      <button onClick={() => { onEdit(itemToEdit); setItemToEdit(null); }} className="w-full h-[64px] bg-[#0F4E0E] text-white rounded-[1.6rem] font-black text-xs uppercase tracking-[0.2em] shadow-xl active:scale-95 transition-all">Guardar Cambios</button>
                  </div>
              </div>
          </div>
        </ModalPortal>
      ...
      {showAddModal && (
        <ModalPortal>
          <div className="fixed inset-0 z-[5000] bg-black/40 backdrop-blur-xl flex items-center justify-center p-4" onClick={() => setShowAddModal(false)}>
              <div className="w-full max-w-[420px] bg-white rounded-[2.8rem] p-10 shadow-2xl relative animate-slide-up" onClick={e => e.stopPropagation()}>
                  <div className="flex justify-between items-center mb-8"><h2 className="text-[#0F4E0E] text-[2rem] font-black tracking-tight leading-none">Nuevo Item</h2><button onClick={() => setShowAddModal(false)} className="p-2.5 text-gray-200 hover:text-black transition-colors"><X className="w-7 h-7" /></button></div>
                  <div className="space-y-6">
                      ...
                      <div className="grid grid-cols-2 gap-4">
                          ...
                          <div className="flex flex-col">
                              <InputLabel icon={Clock}>Caducidad</InputLabel>
                              <div className="relative">
                                  <input type="text" className={MODAL_INPUT_CLASSES + " !text-[#0F4E0E]"} placeholder="AAAA-MM-DD" value={newItem.expires_at || ""} onChange={e => setNewItem({...newItem, expires_at: e.target.value})} />
                      ...
                      <button onClick={handleAddNewItem} disabled={!newItem.name} className="w-full h-[64px] bg-[#0F4E0E] text-white rounded-[1.6rem] font-black text-xs uppercase tracking-[0.2em] shadow-xl active:scale-95 transition-all disabled:opacity-40">AÑADIR A DESPENSA</button>
                  </div>
              </div>
          </div>
        </ModalPortal>
