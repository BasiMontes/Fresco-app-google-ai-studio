...
export const Pantry: React.FC<PantryProps> = ({ items, onRemove, onAdd, onUpdateQuantity, onAddMany, onEdit, isOnline = true }) => {
  ...
  const getExpiryStatus = (item: PantryItem) => {
    ...
    if (days === 0) return { type: 'priority', label: 'Hoy', icon: AlertTriangle, color: 'text-red-600' };
    if (days <= 3) return { type: 'priority', label: `Ene ${format(expiry, 'd', { locale: es })}`, icon: Clock, color: 'text-orange-500' };
  ...
  return (
    <div className="space-y-6 animate-fade-in pb-48 w-full max-w-full px-2 md:px-4">
      ...
      <header className="flex flex-col lg:flex-row justify-between items-start lg:items-center gap-6 pt-4">
        <div>
            ...
            <p className="fresco-body mt-2 !text-lg opacity-40">Tu cocina inteligente, bajo control.</p>
        </div>
        ...
        <div className="flex flex-col sm:flex-row gap-3 w-full lg:w-auto">
            ...
            <div className="flex gap-2">
                <button onClick={() => isOnline && setShowScanner(true)} className="flex-1 sm:flex-none px-6 py-3.5 bg-orange-500 text-white rounded-2xl flex items-center justify-center gap-2 fresco-label !text-white shadow-xl shadow-orange-500/10 active:scale-95 transition-all !text-[10px]">
                    <Camera className="w-4 h-4" /> ESCANEAR
                </button>
                <button onClick={() => setShowAddModal(true)} className="flex-1 sm:flex-none px-6 py-3.5 bg-[#013b33] text-white rounded-2xl flex items-center justify-center gap-2 fresco-label !text-white shadow-xl shadow-[#013b33]/10 active:scale-95 transition-all !text-[10px]">
      ...
      <div className="flex gap-2 overflow-x-auto no-scrollbar pb-2">
          {[{id:'all',l:'TODO EL STOCK'},{id:'expired',l:'CADUCADOS'},{id:'priority',l:'PRIORITARIOS'},{id:'fresh',l:'EN BUEN ESTADO'}].map(f => (
              <button key={f.id} onClick={() => { setActiveFilter(f.id as any); setVisibleLimit(ITEMS_PER_PAGE); }}
                className={`px-6 py-3 rounded-2xl fresco-label transition-all border whitespace-nowrap !text-[9px] ${activeFilter === f.id ? 'bg-[#013b33] border-[#013b33] !text-white shadow-md' : 'bg-white border-gray-50 !text-gray-400 hover:border-[#013b33]/10'}`}
              >
                  {f.l}
              </button>
          ))}
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
          {visibleItems.length === 0 ? (
            ...
            visibleItems.map(item => {
                ...
                return (
                    <div key={item.id} className="bg-white rounded-[2.5rem] border border-gray-50 p-7 shadow-[0_8px_30px_rgb(0,0,0,0.02)] hover:shadow-[0_20px_60px_rgb(0,0,0,0.06)] transition-all duration-500 group flex flex-col h-[360px] animate-fade-in relative">
                        ...
                        {/* Middle Section: Icon circle and Info text */}
                        <div className="flex-1 flex items-center gap-6">
                            ...
                            <div className="space-y-1">
                                <div className={`flex items-center gap-1.5 font-black text-sm tracking-tight ${status.color}`}>
                                    <Clock className="w-3.5 h-3.5" />
                                    <span>{status.label}</span>
                                </div>
                                <div className="fresco-label !text-[9px] !font-black !text-[#013b33]/20 tracking-[0.25em] uppercase">
                                    {catInfo.label}
                                </div>
                            </div>
                        </div>

                        {/* Bottom Section: Quantity Pill Selector */}
                        <div className={`mt-auto rounded-[2.5rem] p-2 flex items-center justify-between border transition-all duration-300 ${isLowStock ? 'bg-[#FFF5F5] border-[#FFEBEB]' : 'bg-gray-50 border-gray-50'}`}>
                            <button onClick={() => onUpdateQuantity(item.id, -1)} className="w-11 h-11 flex items-center justify-center bg-white rounded-full shadow-sm text-gray-300 hover:text-red-500 active:scale-90 transition-all font-black text-xl border border-gray-50/50">-</button>
                            
                            <div className="flex flex-col items-center">
                                <span className={`text-2xl font-black leading-none ${isLowStock ? 'text-[#FF4D4D]' : 'text-[#013b33]'}`}>{item.quantity}</span>
                                <span className={`fresco-label !text-[8px] !font-black mt-1.5 tracking-[0.1em] ${isLowStock ? 'text-[#FF4D4D]/50' : 'text-[#013b33]/20'}`}>
                                    {isLowStock ? 'LOW STOCK' : item.unit.toUpperCase()}
                                </span>
                            </div>
                            
                            <button onClick={() => onUpdateQuantity(item.id, 1)} className="w-11 h-11 flex items-center justify-center bg-teal-600 text-white rounded-full shadow-lg hover:bg-teal-700 active:scale-90 transition-all">
      ...
      {visibleLimit < filteredItems.length && (
          <div className="flex justify-center pt-8 animate-fade-in">
              <button 
                ...
                className="group flex items-center gap-4 px-8 py-4 bg-white border border-gray-100 rounded-[2rem] shadow-lg hover:shadow-xl hover:-translate-y-1 transition-all"
              >
                  <div className="flex flex-col text-left">
                      <span className="fresco-label !text-[#013b33] !text-[9px] mb-1">Cargar más</span>
      ...
      {/* Modals: Simple and Styled */}
      {showAddModal && (
        <div className="fixed inset-0 z-[5000] bg-[#013b33]/20 backdrop-blur-2xl flex items-center justify-center p-4">
            <div className="w-full max-w-sm bg-white rounded-[3.5rem] p-10 shadow-2xl relative animate-slide-up">
